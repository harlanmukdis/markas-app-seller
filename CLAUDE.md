...
**To actually run the app against the backend, use macOS, not Chrome.** The API
sends no CORS headers (see below), so the web build cannot reach it
cross-origin; a native build has no such restriction.

```bash
flutter run -d macos                              # the one target that can call the API today
...
1. **`docs/18-frontend-integration-guide.md`** — written by the backend team for exactly this app. Module→app map, the seed accounts, the core buyer and seller flows, and a list of traps each marked `[terverifikasi]` where it was reproduced against a running server. Start here.
2. **`postman/Marketplace-API.postman_collection.json`** (223 requests, 32 folders) — the real request bodies, and the tiebreaker whenever a field name is in doubt.
3. **`application/config/routes.php`** — the only complete list of what is actually dispatchable.
4. `docs/03-api-documentation.md` — the original design doc. Useful for intent, wrong often enough about specifics that it loses every disagreement.

Note that the backend team is shipping to this repo continuously; `git log` there is the fastest way to see what changed under the app since the last session.
...
What is implemented: auth (register → verify → login, refresh, logout), the account profile, store list/create/settings and the active-store context, file upload, the **product catalogue** (categories, products, variants, publish/unpublish), **store verification** (submit, documents, status), **warehouses & inventory** (warehouses, stock in/out, adjustments, transfers, audits, the movement ledger), the store's **courier selection**, and **orders** (queue, detail, accept/pack/ship, cancel, tracking, refund decisions) — eight services in all: `auth_service`, `store_service`, `media_service`, `catalog_service`, `verification_service`, `inventory_service`, `shipping_service`, `order_service`.

That covers the whole core seller journey: open a shop, get verified, list products, stock them, and work the orders that arrive. **The store wallet is the next gap** — a seller can earn but not yet see or withdraw the money. After that: staff and RBAC, promotions, chat, reviews. Roughly 42 of the ~116 seller-relevant endpoints are wired.
...
- `test/widget_test.dart` (the Flutter counter template, which failed) has been **replaced**. `flutter test` now runs 79 real tests covering the tolerant JSON parsers, model parsing against payloads captured from the live server, and envelope/pagination/error handling. It is a genuine signal — keep it green.
...
The marketplace API exposes 33 modules across 207 dispatchable routes (`application/config/routes.php`: 206 under `/api/v1` plus the public `/r/{shortCode}` affiliate redirect), of which the Postman collection covers 223; the seller app needs a fraction of them. Build in this order, since each depends on the last: **store verification** (submit + documents, using `/media/upload`) -> **product catalog** (categories, products, variants) -> **warehouses & inventory** (stock-in/out, adjustments, audits, movements) -> **orders** (accept, pack, ship, cancel, tracking, refund requests) -> **store wallet** (balance, withdraw) -> chat, promotions, reviews, and the advanced modules.

**`routes.php` is the ground truth, not `docs/03-api-documentation.md`.** The doc describes a design; the routes file describes what is dispatchable. Endpoints that exist only in the doc: `POST /products/{id}/images` and the whole bundle and subscription surface (`/stores/{id}/bundles`, `/products/{id}/subscriptions`) are unrouted and answer 404, while `DELETE /products/{id}` is routed but unimplemented and answers 405 — there is no way to delete or archive a product through the API at all. Where the doc and the routes both have an endpoint but disagree on a field name, the **Postman collection** is the one that matches the code.
...
- 🔴 **Warehouse create and update are not whitelisted.** The controller passes the request body straight into the SQL insert/update, so a single key that is not a column answers `500` with an HTML "Database Error" page. Send exactly `name`, `address`, `city`, `province`, `postal_code`, `latitude`, `longitude`, `status` and nothing else. `is_default` is pointless to send: the server makes the store's first warehouse the default and every later one not, whatever the request said.
