...
- **`GET /offers` is paginated for buyers but not for a `SEL` token** — the seller branch returns everything and carries no `meta`. Never send `page`/`per_page` or read `meta.total_pages` from the seller app.
